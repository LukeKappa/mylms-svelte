<script lang="ts">
  ...
  function renderMathInHtml(html: string): string {
    ...
    processed = processed.replace(/\$\$([\s\S]+?)\$\$/g, (_, tex) => safeRender(tex, true));
    ...
    processed = processed.replace(/\\\((.+?)\\\)/g, (_, tex) => safeRender(tex, false));
    processed = processed.replace(/\\\[([\s\S]+?)\\\]/g, (_, tex) => safeRender(tex, true));
    processed = processed.replace(/\\begin\{equation\}([\s\S]+?)\\end\{equation\}/g, (_, tex) => safeRender(tex.trim(), true));
    processed = processed.replace(/\\begin\{align\*?\}([\s\S]+?)\\end\{align\*?\}/g, (_, tex) => safeRender(`\\begin{aligned}${tex.trim()}\\end{aligned}`, true));
  ...
  function processHtmlForTOC(html: string, uniquePrefix: string) {
    ...
    const processedHtml = html.replace(/\<(h[1-6])([^\>]*)\>(.*?)\<\/\1\>/gi, (match, tag, attrs, contentStr) => {
  ...
  async function fetchContent(url: string) {
    ...
    try {
      ...
      if (!result.success || result.error) {
        ...
        dbService.saveActivity(url, result.content).catch((e) => console.error('Failed to save to DB:', e));
      }
    } catch (err) {
  ...
  async function handleContentClick(e: MouseEvent) {
    ...
    if (link && link.href) {
      ...
      if (isMoodleUrl(link.href)) {
        ...
        const contentArea = (e.currentTarget as HTMLElement).querySelector('.activity-content');
        if (contentArea) {
  ...
  function setIsLoading(val: boolean) { isLoading = val; }
  function setError(val: string | null) { error = val; }
  function setContent(val: string | null) { content = val; }
  function setToc(val: typeof toc) { toc = val; }
</script>
...
<div class="border-b border-neutral-800 last:border-b-0">
  ...
  >
    <div class="p-2 bg-neutral-800 rounded-lg text-neutral-400 group-hover:bg-white group-hover:text-black transition-colors flex-shrink-0">
      <Icon size={20} />
    </div>
    
    <div class="flex-1 min-w-0">
      <p class="text-white font-medium">{activity.name}</p>
      <p class="text-xs text-neutral-500 uppercase tracking-wider">{activity.type}</p>
    </div>
  ...
  {#if isExpanded}
    ...
    <div class="border-t border-neutral-800/50" onclick={handleContentClick}>
      ...
      {:else if content}
        <div class="relative">
          {#if toc.length > 0}
            <div id="table-of-contents" class="mb-6 p-4 bg-neutral-800/50 rounded-lg border border-neutral-800">
              <p class="text-sm font-medium text-neutral-400 mb-2 uppercase tracking-wider">Table of Contents</p>
              <ul class="space-y-1">
                {#each toc as item}
          ...
          <div 
            class="html-content p-4 bg-neutral-900 w-full"
          >
