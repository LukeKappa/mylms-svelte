...
<div class="flex flex-col gap-3 w-full md:w-auto">
  <div class="flex items-center gap-2 flex-wrap md:flex-nowrap">
    ...
    <div class="relative">
      ...
      {#if isFilterOpen}
        <div
          class="absolute right-0 mt-2 w-72 bg-neutral-900 border border-neutral-800 rounded-xl shadow-2xl z-50 overflow-hidden"
        >
          <div
            class="p-3 border-b border-neutral-800 flex justify-between items-center bg-neutral-900"
          >
      ...
      >
        <div class="relative">
          ...
          {#if visibleActivityTypes.size > 0}
            <span
              class="absolute -top-2 -right-2 w-4 h-4 bg-blue-600 text-[10px] flex items-center justify-center rounded-full border border-neutral-900"
            >
        ...
        >
          ...
          >
            <span class="text-sm font-medium text-white"
              >Select Activity Types</span
            >
            <button
              onclick={onToggleAllActivityTypes}
              class="text-xs text-blue-400 hover:text-blue-300"
            >
              {visibleActivityTypes.size === 6
                ? "Reset to Notes"
                : "Select All"}
            </button>
          </div>
            ...
            {#each activityTypes.filter( (type) => availableActivityTypes.includes(type.id), ) as type (type.id)}
              ...
              >
                ...
                >
                  ...
                  {#if type.default}
                    <span
                      class="text-[10px] px-1.5 py-0.5 bg-green-900/30 text-green-400 rounded border border-green-800"
                    >
                      DEFAULT
                    </span>
                  {/if}
  ...
  {#if isSyncing}
    <div class="w-full">
      <div
        class="flex items-center justify-between text-xs text-neutral-400 mb-1.5"
      >
        <span
          >Downloading {totalActivities}
          {totalActivities === 1 ? "activity" : "activities"}...</span
        >
      ...
      <div
        class="h-2 bg-neutral-800 rounded-full overflow-hidden border border-neutral-700"
      >
        <div
          class="h-full bg-gradient-to-r from-neutral-400 via-neutral-300 to-neutral-400 animate-pulse relative"
        >
