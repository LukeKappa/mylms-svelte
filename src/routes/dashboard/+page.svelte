<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      ...
      if ((e as any).message?.includes('401') || (e as any).message === 'Invalid token') {
...
{:else}
  <div class="min-h-screen bg-black">
    <!-- Top Navigation Bar -->
    <header class="sticky top-0 z-40 w-full border-b border-neutral-800 bg-black/50 backdrop-blur-xl">
      <div class="container mx-auto px-4 h-16 flex items-center justify-between">
        <div class="flex items-center gap-4">
          <a href="/dashboard" class="text-xl font-bold text-white tracking-tight">MyLocalMS</a>
          <div class="hidden md:block w-px h-6 bg-neutral-800"></div>
          <CommandPalette allCourses={courses.map(c => ({ id: c.id, fullname: c.fullname, shortname: c.shortname }))} />
        </div>
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <div class="w-8 h-8 rounded-full bg-neutral-800 flex items-center justify-center border border-neutral-700">
